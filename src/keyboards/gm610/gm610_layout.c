 ...
 * PERSISTANCE DU MODE.
 *
 * `user_settings.ul_speed` est libre sur cette carte : elle n'a pas d'underglow,
 * et son moteur d'éclairage ne lit jamais ce champ. On s'en sert pour persister
 * le mode, plutôt que d'allonger `user_settings_t` -- ce qui ferait retomber
 * TOUS les réglages de toutes les cartes au prochain flash.
 *
 * ⚠️ `ul_effect` est DÉJÀ pris sur cette carte : kb.c y persiste le mode de
 * liaison (USB / sans-fil). Ne pas les confondre.
 */
#define GM610_SETTINGS_AZ user_settings.ul_speed
...
void gm610_layout_toggle(void)
{
    az_on = !az_on;
    az_reset();

    GM610_SETTINGS_AZ = az_on ? 1u : 0u;
    settings_mark_dirty(); /* `settings_task()` écrit la flash au tour suivant */

    gm610_layout_announce(az_on ? true : false, false);
}

void gm610_layout_restore(bool on)
{
    az_on = on ? 1 : 0;
    gm610_layout_announce(on, true); /* true = annonce longue, au démarrage */
}

void gm610_layout_euro(void)
{
    az_push(KC_E, AZ_AG);
}

bool gm610_layout_intercept(uint16_t qcode, bool pressed)
{
